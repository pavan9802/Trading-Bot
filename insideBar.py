...
def get_insideBar(bars):
    return bars.high[-2] > bars.high[-2] and bars.low[-2] < bars.low[-1]


def buy(ticker, bars):
    while True:
        quotes = api.get_quotes(ticker, limit=10).df
        ...
        if (avg > bars.high[-1]):
            ...
            api.submit_order(ticker, notional=notionalQTY,
                             side='buy', time_in_force='day', trail_percent="2",
                             stop_loss=dict(
...
while True:
    for ticker in SYMBOLS:
        notionalQTY = float(api.get_account().buying_power) * 0.05
        # GET DATA
        bars = api.get_bars(ticker, TimeFrame.Minute).df
